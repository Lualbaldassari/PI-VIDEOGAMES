...
const infoApi = async () => {
  ...
  try {
    for (let i = 1; i < 6; i++) {
      //con un for recorro mi API, ya que es un arreglo, 5 veces
      ...
      respuesta.data.results.map((v) => {
        //a la respuesta/resultado lo mapeo
        videojuegos.push({
          //y pusheo en mi array vacio todo aquello que mapee
          id: v.id,
          name: v.name,
...
const nameApi = async (name) => {
  const infoSearch = await axios.get(`${URL}?search=${name}&key=${APIKEY}`);
...
const idApi = async (id) => {
  try {
    const rtaApi = await axios.get(`${URL}/${id}?key=${APIKEY}`);
    if (rtaApi) {
